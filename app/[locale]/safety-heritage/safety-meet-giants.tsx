...
const DECORATIVE = "font-['Libre_Baskerville'] italic tracking-[-0.03em]"
...
export async function SafetyMeetGiants() {
  ...
  return (
    <section className="w-full flex flex-col" style={{ background: 'linear-gradient(180deg, #dae2e8 0%, #7B95AB 13.51%, #9497AD 29.86%, #000000 68.01%)' }}>

      <div className="flex flex-col items-center text-center gap-4 pt-[clamp(60px,7vw,120px)] pb-[clamp(40px,5vw,80px)] px-4">
        <h2>{t('title')}</h2>
        <p className="max-w-229">{t('description')}</p>
      </div>

      <div className="px-4 md:px-[clamp(40px,6.94vw,100px)] pb-[clamp(40px,5vw,80px)]">

        <div className="hidden md:flex flex-row gap-[clamp(16px,4.72vw,68px)] max-w-308 pt-[clamp(32px,4vw,60px)] pb-8">
          {[
            { label: t('subtitle'), desc: t('description2'), flex: 'flex-1' },
            { label: t('subtitle2'), desc: t('description3'), flex: 'flex-[2]' },
          ].map(({ label, desc, flex }) => (
            <div key={label} className={`flex flex-col gap-2 ${flex}`}>
              <h4>{label}</h4>
              <span className={`${DECORATIVE} not-italic text-[clamp(20px,1.67vw,24px)] leading-7.5`}>{desc}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-[clamp(16px,4.72vw,68px)] max-w-308">
      ...
      <div className="flex flex-col gap-6 p-[clamp(24px,3.47vw,50px)] sm:mx-10">
        <div className="flex flex-col gap-2">
          <h4 className="text-background">{banner.title}</h4>
          <span className={`${DECORATIVE} not-italic text-xl leading-7.5 text-background`}>{banner.subtitle}</span>
        </div>

        <div className="relative w-full h-110 rounded-(--radius) overflow-hidden">
          <CloudinaryImage publicId={IMAGES.safety.meetGiants.banner} alt={banner.cardtitle} fill className="object-cover object-center" />
          <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0) 0%, rgba(0,0,0,0.31) 72.91%)' }} />
          <div className="absolute bottom-0 left-0 p-[clamp(16px,2.22vw,32px)] flex flex-col gap-2">
            <span className={`${DECORATIVE} text-4xl leading-[1.234] text-background`}>{banner.cardtitle}</span>
            <p className="text-background text-md font-medium">{banner.cardsubtitle}</p>
          </div>
        </div>

        <div className="flex flex-col gap-6 mb-30 mt-6">
          {bannerItems.map((item, i) => (
            <div key={i} className="flex flex-row items-start gap-3">
              <div className="shrink-0 w-12 h-8 mt-0.5">{item.icon}</div>
              <div className="flex flex-col">
                <span className="font-bold text-[14px] leading-4.5 text-background">{item.name}</span>
                <span className="text-[14px] leading-4.5 text-background/80 font-medium">{item.desc}</span>
              </div>
            </div>
          ))}
        </div>
...
function MobileLabel({
  ...
  return (
    <div className="flex flex-col items-center text-center gap-4 py-10 md:hidden">
      ...
      <span
        className={`${DECORATIVE} not-italic text-xl leading-6 tracking-[-0.03em] ${colorClass}`}
      >
        {desc}
      </span>
    </div>
...
function BalloonCard({ image, card }: { image: string; card: { name: string; subname: string; description: string; capacity: string; mission: string } }) {
  return (
    <div className="flex flex-col gap-6 w-full md:flex-1">
      <div className="relative w-full h-110 rounded-[22px] overflow-hidden shrink-0">
        <CloudinaryImage publicId={image} alt={card.name} fill className="object-cover object-center" />
        <div className="absolute inset-0">
          <div
            className="absolute bottom-0 left-0 w-full h-1/3 p-[clamp(1rem,2.22vw,2rem)] flex flex-col justify-start gap-2"
            style={{
              background:
                'linear-gradient(180deg, rgba(0,0,0,0) 0%, rgba(0,0,0,0.40) 100%)',
            }}
          >
            <span className={`${DECORATIVE} text-2xl text-background text-left`}>
              {card.name}
            </span>
      ...
      <span className="text-background text-xs md:text-sm leading-5">{card.description}</span>
      <div className="flex flex-col gap-3 text-background text-xs md:text-sm leading-5">
...
function TargetIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="shrink-0">
      <path d="M23.175 4.20011C23.1 3.90011 22.8 3.75011 22.5 3.75011H20.25V1.50011C20.25 1.20011 20.1 0.900109 19.8 0.825109C19.5 0.675109 19.2 0.750109 18.975 0.975109L15.975 3.97511C15.825 4.12511 15.75 4.27511 15.75 4.50011V7.20011L11.475 11.4751C11.175 11.7751 11.175 12.2251 11.475 12.5251C11.625 12.6751 11.85 12.7501 12 12.7501C12.15 12.7501 12.375 12.6751 12.525 12.5251L16.8 8.25011H19.5C19.725 8.25011 19.875 8.17511 20.025 8.02511L23.025 5.02511C23.25 4.80011 23.325 4.50011 23.175 4.20011Z" fill="white"/>
      <path d="M13.575 13.575C13.2 14.025 12.6 14.25 12 14.25C11.4 14.25 10.8 14.025 10.425 13.575C9.525 12.675 9.525 11.25 10.425 10.425L12.525 8.325C12.375 8.25 12.15 8.25 12 8.25C9.9 8.25 8.25 9.9 8.25 12C8.25 14.1 9.9 15.75 12 15.75C14.1 15.75 15.75 14.1 15.75 12C15.75 11.85 15.75 11.625 15.675 11.475L13.575 13.575Z" fill="white"/>
      <path d="M21.075 9.075C20.7 9.525 20.1 9.75 19.5 9.75H17.4L16.875 10.275C17.1 10.8 17.175 11.4 17.175 12C17.175 14.925 14.85 17.25 11.925 17.25C9 17.25 6.675 14.925 6.675 12C6.675 9.075 9 6.75 11.925 6.75C12.525 6.75 13.125 6.9 13.65 7.05L14.25 6.6V4.5C14.25 3.9 14.475 3.3 14.925 2.925L15.675 2.175C14.475 1.725 13.275 1.5 12 1.5C6.225 1.5 1.5 6.225 1.5 12C1.5 17.775 6.225 22.5 12 22.5C17.775 22.5 22.5 17.775 22.5 12C22.5 10.725 22.275 9.525 21.825 8.325L21.075 9.075Z" fill="white"/>
    </svg>
  )
}
...
function HeartIcon() {
  return (
    <svg width="22" height="21" viewBox="0 0 20 19" fill="none" className="shrink-0">
