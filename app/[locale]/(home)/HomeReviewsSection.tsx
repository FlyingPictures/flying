...
const CARD_STYLES = { width: 'clamp(345px, 40vw, 537px)', height: 'clamp(280px, 28vw, 356px)', borderRadius: '22px' };

const AVATAR_CLASSES = `relative w-[32px] h-[32px] lg:w-[72px] lg:h-[72px] rounded-full overflow-hidden`;
    ...
    <section className="relative w-full overflow-hidden" style={{ height: '1386px' }}>
      ...
      <div className="relative mx-auto px-4 pt-[100px] h-full">
        ...
        <div className="lg:hidden flex flex-col items-center gap-6 mt-36 pb-8 h-[900px]">
          {reviewsData.slice(0, 3).map((review) => <ReviewCard key={review.id} review={review} />)}
        </div>
      </div>
    </section>
  );
      ...
      <div className="flex items-center justify-center gap-[clamp(16px,4vw,40px)] h-[clamp(40px,6vw,60px)]">
        {PLATFORM_KEYS.map((key) => (
          <div key={key} className="flex items-center justify-center h-full">
            <CloudinaryImage publicId={IMAGES.home.reviews.platforms[key]} alt={`${key} 5 star reviews`} width={500} height={150} className="h-full w-auto object-contain" priority={key === 'google'} />
          </div>
        ))}
      </div>
    </div>
...
function DesktopColumn({ children, leftAlign }: { children: React.ReactNode; leftAlign?: boolean }) {
  const alignment = leftAlign ? 'items-end' : 'justify-center -translate-y-[120px]';
  return <div className={`flex flex-col gap-[24px] w-full ${alignment}`}>{children}</div>;
}
...
function ReviewCard({ review }: { review: Review }) {
  return (
    <article className="bg-card p-6 flex flex-col" style={CARD_STYLES}>
      ...
      <div className="lg:hidden mt-auto self-start">{STAR_RATING}</div>
    </article>
  );
}

function GhostCard({ position }: { position: 'top' | 'bottom' }) {
  return <div style={{ ...CARD_STYLES, background: GHOST_CARD_GRADIENTS[position] }} />;
}
