...
export async function WhyFlightWhitUs() {
  ...
  return (
    <section className="relative w-full bg-black overflow-hidden min-h-610 lg:min-h-420 text-background">
      <div className="absolute inset-x-0 top-0 h-400 lg:h-full">
        <CloudinaryImage
          publicId={IMAGES.home.whyChoose.background}
          alt=""
          fill
          sizes="100vw"
          className="object-cover object-[calc(50%+50px)_top] lg:object-top"
          priority
        />
      </div>

      <div className="absolute left-1/2 -translate-x-1/2 w-[clamp(345px,90vw,1240px)] lg:px-0 bottom-0 lg:bottom-56 lg:h-[clamp(520px,60vw,785px)] pb-[clamp(24px,6vw,40px)] lg:pb-0">
        <div className="flex flex-col lg:flex-row w-full items-start">
          <div className="hidden lg:block w-full lg:w-1/2">
            <h2 className="text-left!">{t("title")}</h2>
          </div>
          <div className="w-full lg:w-1/2 flex flex-col">
            <h4 className="text-center lg:text-left">{t("takeAway")}</h4>
            <h2 className="text-left lg:hidden mb-6">{t("title")}</h2>
            <p className="px-6 py-6 lg:px-0">{t("await")}</p>
          </div>
        </div>
