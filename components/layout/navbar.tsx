...
import { EqualsIcon, XIcon, WhatsappLogoIcon, HeadsetIcon, GlobeSimpleIcon } from "@phosphor-icons/react"
...
const ExperienceLink = ({ id, href, onClick, className }: { id: string, href: string, onClick?: () => void, className?: string }) => {
  const t = useTranslations("nav")
  return <Link href={href} onClick={onClick} className={cn("font-poppins font-medium text-secondary hover:opacity-70 transition-opacity outline-none", className)}>{t(id)}</Link>
}

const BookButton = ({ className }: { className?: string }) => {
  const t = useTranslations("nav")
  return <Button variant="primary" size="sm" className={cn("px-8 font-bold", className)}>{t("bookFlight")}</Button>
}
...
const LanguageSwitcher = ({ className }: { className?: string }) => {
  ...
  const { nextLocale, label } = useMemo(() => ({
    nextLocale: locale === "en" ? "es" : "en", label: locale === "en" ? "Language" : "Idioma"
  }), [locale])
...
export default function Navbar() {
  ...
  const isHeaderHidden = scrollY > 50 && scrollDirection === "down" && !isSheetOpen
    ...
    <>
      <header className={cn("hidden lg:flex flex-col items-center z-50 fixed top-0 w-full transition-transform duration-500", !isHeaderVisible && "-translate-y-[11rem]")}>
        <div className="w-full bg-destructive text-background font-bold h-12 flex items-center justify-center px-4 overflow-hidden">
          <a href={`tel:${CONTACT.PHONE}`} className="uppercase whitespace-nowrap" style={{ fontSize: "clamp(0.65rem, 2.5vw, 0.875rem)" }}>
            {useTranslations("banner")("company")} {CONTACT.DISPLAY}
          </a>
        </div>
        ...
        <div className="w-[95%] max-w-[92rem] mt-4">
          <nav className="relative bg-background h-[5.125rem] rounded-2xl shadow-2xl flex items-center justify-between px-8">
            ...
            <div className="flex flex-1 items-center" style={{ gap: "clamp(0.25rem, 2vw, 2rem)" }}>
              <div 
                      ...
                      {EXPERIENCES.map(exp => (
                        <DropdownMenu.Item key={exp.id} asChild onSelect={() => setIsMenuOpen(false)}>
                          <div className="outline-none">
                            <ExperienceLink id={exp.id} href={exp.href} className="block px-4 py-2 text-[0.95rem] hover:bg-secondary/5 rounded-md" />
                          </div>
                        </DropdownMenu.Item>
                      ))}
                    </DropdownMenu.Content>
            ...
            <div className="flex flex-1 items-center justify-end" style={{ gap: "clamp(0.25rem, 2vw, 2rem)" }}>
      ...
      <header className={cn("lg:hidden fixed top-0 inset-x-0 z-50 transition-transform duration-500", isHeaderHidden && "-translate-y-[8rem]")}>
        ...
        <nav className="relative h-[4.5rem] bg-surface px-6 flex items-center justify-between shadow-md">
          <Link href="/" onClick={() => setIsSheetOpen(false)} className="absolute top-0 left-6 z-10">
          ...
          <div className="flex items-center gap-3 ml-auto">
            ...
            <SheetPrimitive.Root open={isSheetOpen} onOpenChange={setIsSheetOpen}>
              <SheetPrimitive.Trigger asChild>
                <button className="p-1 outline-none"><EqualsIcon size={32} weight="bold" className="text-secondary" /></button>
              ...
              <SheetPrimitive.Portal>
                <SheetPrimitive.Content className="fixed inset-y-0 right-0 z-[60] w-full bg-surface flex flex-col shadow-xl animate-in slide-in-from-right duration-300 outline-none border-none">
                  ...
                  <div className="flex flex-col flex-1 px-10 py-10 gap-5 overflow-y-auto">
                    <h2 className="font-inter font-bold text-secondary text-[1.5rem] tracking-tight">{t("flightExperiences")}</h2>
                    <div className="flex flex-col gap-4 ml-1">
                      {EXPERIENCES.map(exp => (
                        <ExperienceLink key={exp.id} id={exp.id} href={exp.href} className="text-[1.15rem]" onClick={() => setIsSheetOpen(false)} />
                      ))}
                    </div>
                    ...
                    <div className="mt-auto pb-8 flex flex-col gap-6">
                      <Link href="/contact" className="font-inter font-bold text-secondary flex items-center gap-2 text-[1rem] tracking-tight" onClick={() => setIsSheetOpen(false)}>
                        <HeadsetIcon size={24} weight="bold" /> {t("contactSupport")}
                      </Link>
                      <LanguageSwitcher />
                    </div>
                  </div>
